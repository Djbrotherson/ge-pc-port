...
- **Also bites RAW hardcoded struct-stride writes, not just Gfx/Vtx.**
  ...
  the N64 `bg_portal_data_entry` is 8B (`ptr@0, cr1@4..cb2@7`), but on PC
...
- **D132/D126 corollary; a zeroed ROM-serialized pointer-width tail slot is not
  ...
  serialized tail pointer, zeroed by `tools_pc/d88_emit.py`'s `emit_pad()`),
  but every `PadRecord` in every checked setup table (`UsetupdamZ.c` etc.) initialises
...
- **D152 addendum (M-31 static audit).** Every `osSetIntMask(OS_IM_NONE)` in
  ...
  first; `alEvtqFlush`, `alEvtqFlushType` likewise), `csplayer.c` `__CSPRepostEvent`,
